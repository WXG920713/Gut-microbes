...
p<-ggplot(plot_df, aes(x=PC1, y=PC2, color=group, fill=group)) +
  ...
  labs(title = "Total PCOA based on Bray-Curtis distance
"
  ) +
  ...
  theme(
    ...
    axis.text = element_text(size = 15, color = "black", face = "bold"),  # 修改x,y轴刻度大小和颜色
    axis.title = element_text(color = "black", size = 14),  # 修改x,y轴标题颜色和字体大小
...
ggsave("Total-PCOA.svg", plot = p, width = 5, height = 4)   
