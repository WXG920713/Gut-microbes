...
p <- ggplot(data_long, aes(x = variable, y = value)) +
  ...
  theme(
    axis.text.x = element_text(size = 10, color = 'black', angle = 45, hjust = 1, face = "italic"),
    axis.text.y = element_text(size = 10,  color = 'black'),
    panel.border = element_rect(colour = "black", fill = NA, size = 1),
    legend.position = "none")

p1<-p + 
  stat_pvalue_manual(stat.test, x = "variable", label = "p.adj", y.position = 29, position = position_dodge(width = 1),size=3) +
  geom_segment(data = stat.test, aes(x = as.numeric(variable) - 0.4, xend = as.numeric(variable) + 0.4, y = 28.5, yend = 28.5)) +
  geom_segment(data = stat.test, aes(x = as.numeric(variable) - 0.4, xend = as.numeric(variable) - 0.4, y = 28.5, yend = 28.2)) +
  geom_segment(data = stat.test, aes(x = as.numeric(variable) + 0.4, xend = as.numeric(variable) + 0.4, y = 28.5, yend = 28.2))
...
ggsave('ET-L.svg',width=10,height=7, plot = p3)
